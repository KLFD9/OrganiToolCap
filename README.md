...
Application web pour créer, modifier à plusieurs et exporter des organigrammes modernes. Conçue pour une équipe marketing non technique : le mode local reste sans compte et fichier-first ; le réseau n'est utilisé qu'après l'activation volontaire d'une session collaborative. Aucune télémétrie.
...
- **Collaboration volontaire** : « Partager » crée une session WebRTC temporaire par lien. Le document est synchronisé entre participants, tandis que curseurs, sélections et pseudonymes restent éphémères et ne sont jamais ajoutés au fichier.
...
- **Partage en direct** : lien sans compte, pseudonyme, participants visibles, curseurs et sélections colorés, fusion des modifications et reprise locale de session. Les conflits de hiérarchie sont réparés automatiquement : parent unique, anti-cycle et liens fonctionnels conservés.
...
- **Import Excel / CSV contrôlé** : ouvrez directement un classeur `.xlsx` ou un export CSV Excel / Google Sheets. Pour Excel, choisissez la feuille, la ligne d’en-tête et la correspondance des colonnes `Nom`, `Poste`, `Pôle`, `E-mail`, `Téléphone` et `Responsable`. Avant toute modification, un aperçu local résume les personnes, responsables, racines et anomalies. Le client choisit ensuite une mise en page automatique orientée page ou l’ordre brut du fichier ; l’import ouvre un nouveau document, reste annulable et ne réutilise jamais silencieusement le chemin du fichier précédent.
- **Comparaison locale de versions** : comparez un ancien `.orgchart.json` au document actuel pour identifier arrivées, départs, changements de poste, de pôle, de coordonnées, de responsable et de rattachements fonctionnels. Le rapprochement privilégie l’identifiant puis l’e-mail et signale les identités ambiguës, sans modifier aucun fichier.
...
- **Identité de page libre** : chaque page peut ajouter, remplacer, masquer ou reprendre le titre et le sous-titre du document, ainsi que remplacer les logos principal et secondaire. Un logo remplacé reprend exactement le gabarit du précédent, puis reste déplaçable et redimensionnable sur la feuille. Le contenu, les images et le style sont enregistrés dans le fichier et reproduits dans les exports PDF/PPTX.
...
Le relais privé, le serveur TURN et les variables Vercel sont détaillés dans
[`docs/DEPLOIEMENT_COLLABORATION.md`](docs/DEPLOIEMENT_COLLABORATION.md).
