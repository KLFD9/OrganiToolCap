...
- **Vue annuaire** : table triable et filtrable de tous les membres (nom, poste, pôle, e-mail, responsable, taille d'équipe), synchronisée avec le canvas — sélectionner une ligne ouvre la fiche dans l'inspecteur, « voir dans l'organigramme » recentre la vue sur la personne. **Export CSV** au format de l'import (round-trip complet : Excel / Google Sheets devient un éditeur de masse).
...
- **PDF vectoriel natif** : par défaut, le PDF est dessiné en vectoriel (cartes, connecteurs et pastilles d'initiales natifs, comme le PowerPoint éditable) — texte parfaitement net à toutes les échelles, fichier d'environ 20-30 Ko au lieu de plusieurs Mo. Le mode « capture image » reste disponible (fidélité au pixel, photos) et sert automatiquement en multi-pages.
